<template>
	<div class="container">
		<div class="border border-warning rounded my-4 event-border">
			<img src="https://i.postimg.cc/rpPTbtD4/event-banner-sm.jpg" class="d-block w-100 img-fluid" alt="">   
			<div class="bg-secondary  text-white ">
				...
				<div class="bg-dark p-3">
					<div class="text-center"> 
						<button class="btn btn-danger" type="button" @click.prevent="getCoupon" v-if="!isCliked">顯示優惠代碼</button> 
						<div class="input-group" v-if="isCliked & !codeGet">
							<input id="codeId" type="text" class="form-control" placeholder="" aria-label="Recipient's username" aria-describedby="button-addon2" v-model="couponCode" disabled>
							<div class="input-group-append">
								<button class="btn btn-success copyBtn" type="button" id="button-addon2" :data-clipboard-text="couponCode" @click="copy('.copyBtn')" >複製代碼</button>
							</div>
						</div>                            
						<router-link to="/products" class="btn btn-danger"  @click.prevent="getCoupon" v-if="isCliked & codeGet">複製成功，繼續購物</router-link> 
					</div>
				</div>
