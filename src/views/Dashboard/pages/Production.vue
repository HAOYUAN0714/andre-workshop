        ...
        <table class="table mt-4">
            ...
            <tbody>
                <tr v-for="item in products" :key="item.id">
                    ...
                    <td>
                        <button class="btn btn-danger btn-sm" @click="delModal(item)">刪除</button>
                    </td>
                </tr>
            </tbody>
        </table>
        <Pagination  :paginationProp='pagination' @emit-update-page='getProduct'></Pagination>
        <!-- 原始分頁內容 
        <nav aria-label="Page navigation example">
            <ul class="pagination">
                <li class="page-item" :class="{'disabled': !pagination.has_pre}"> 如果沒有上1頁，就不能點擊
                    <a class="page-link" href="#" aria-label="Previous" @click.prevent='getProduct(pagination.current_page -1)'> 到上1頁功能即 目前頁碼+1
                        <span aria-hidden="true">&laquo;</span>
                    </a>
                </li>
                <li class="page-item"  v-for="page in pagination.total_pages" :key="page" :class="{'active':pagination.current_page === page}"> 顯示頁面為目前頁面時，加上.active
                    <a class="page-link" href="#" @click.prevent='getProduct(page)'>{{page}}</a></li>   點擊指定頁面，顯示此頁面資料
                <li class="page-item" :class="{'disabled': !pagination.has_next}"> 如果沒有下1頁，就不能點擊
                    <a class="page-link" href="#" aria-label="Next" @click.prevent='getProduct(pagination.current_page +1)'>  到下1頁功能即 目前頁碼+1
                ...
                <div class="modal-body">
                    ...
                    <div class="col-sm-8">
                        ...
                        <div class="form-check">
                            <!-- is_enable = 1 代表啟用 0 代表不啟用 ，設定 :true-value="1" 代表當此 checkbox 的 value為true is_enable要等於 1 :false-value="0" 代表當此 checkbox 的 value為false is_enable要等於 0-->
...
export default {  // 建立取得產品資訊的方法，並用created() 鉤子 啟用這個方法來獲取資料
    data(){
        return{
            ...
            tempProduct:{}, // 用來撈 我們在產品新增、編輯、刪除 的modal的所有項目的input的值 ，重點在於當我們點擊商品的新增、編輯、刪除 ，再接著點到另1個商品的 新增、編輯、刪除 時都要改變tempProduct的值
    ...
    methods:{
        getProduct(page = 1){  // 撈取伺服器回傳給我們的產品資料 ， 之後新增產品、編輯產品、刪除產品，都會用這段做頁面的更新 ，其中變數page用來取得頁碼，並給予預設值 1 讓其他用到 getProduct()方法的地方 即使沒有設定page參數，也能自動給page變數並等於1
        ...
        uploadFile(){
            ...
            vm.$http.post(url,formData,{ // 使用POST方法傳送檔案到url，傳送檔案為formData
                ...
                if(response.data.success){
                    // vm.tempProduct.imageUrl = response.data.imageUrl; // 使用此方法賦值，下面console 會發現 vm.tempProduct 並沒有 set() get() 方法可以使用，這表示這份資料並沒有雙向綁定
        ...
        openModal(isNew,item){
            if(isNew){  // 這是參數的isNew 並不是我們元件data return 的 this.isNew
                ...
                this.tempProduct =Object.assign({},item); // 如果是點擊編輯產品將此產品item的資料傳入此modal this.tempProduct 當前點擊處的資料即item 也就是modal 裡的input 會是有填入資料的狀態
        ...
        updateProduct(){
            ...
            this.$http[httpMethod](api,{data:vm.tempProduct}).then((response) => {  // 將新增產品傳送到後端伺服器 ， 因驗證api的物件外層還有個data 所以用 {data:vm.tempProduct} 包裹物件內容
                console.log(response.data);
