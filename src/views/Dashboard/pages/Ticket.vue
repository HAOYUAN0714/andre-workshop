...
export default {
  ...
  methods: {
    ...
    // 其中變數page用來取得頁碼，並給予預設值 1 讓其他用到 getProduct()方法的地方 即使沒有設定page參數，也能自動給page變數並等於1
    getTickets(page = 1) {
    ...
    openModal(isNew, item) {
      if (isNew) { // 這是參數的isNew 並不是我們元件data return 的 this.isNew
        ...
        // 如果是點擊編輯優惠券將此優惠券item的資料傳入此modal this.newTicket
        // 當前點擊處的資料即item 也就是modal 裡的input 會是有填入資料的狀態
        // this.newTicket = Object.assign({}, item); // 或寫成 { ...item }
        this.newTicket = { ...item }; // ESLint下使用 Object bread寫法
    ...
    updateTicket() {
      ...
      // 將新增優惠券傳送到後端伺服器 ， 因驗證api的物件外層還有個data 所以用 {data:vm.newTicket} 包裹物件內容
      this.$http[httpMethod](api, { data: vm.newTicket }).then((response) => {
        console.log(response.data);
